...
(a) Example distributions of roughness values (Equation \@ref(eqn:roughness)), showing the different extremes in environmental heterogeneity observed in each region when compared at fine (0.05º) and coarse (3QDS) scales. 
...
These distributions of roughness values were compared between the Cape and SWA at each of the four spatial scales, not just 0.05º and 3QDS, using non-parametric Mann-Whitney $U$-tests to test for differences. 
...
```{r roughness, echo=FALSE, fig.pos="H", out.width="18cm", fig.cap="(ref:roughness)"}
knitr::include_graphics(here::here("figures/fig-1-roughness_manual-edit.png"))
```

```{r richness-vs-turnover, echo=FALSE, fig.pos="H", out.width="18cm", fig.cap="(ref:richness-vs-turnover)"}
