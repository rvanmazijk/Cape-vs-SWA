...
import_objects(
  here::here("outputs/06_species-turnover-and-richness/")
)
import_objects(
  here::here("outputs/05_species-turnover-w-distance/"),
  ignore_RDS = TRUE
)
tbl_format <- options("knitr.table.format")
hold_pos_if_latex <- function(x, format = tbl_format) {
  print(glue(
    "Rendering to {format}, so will use kableExtra to prevent table floating"
  ))
  if (format == "pdf") {
    x %<>% kableExtra::kable_styling(latex_options = "hold_position")
  }
  x
}
```
...
data_sources %>%
  knitr::kable(caption = "(ref:data-sources)") %>%
  hold_pos_if_latex()
```
...
species_turnover_geodist_m_tidy %>%
  ...
  knitr::kable(caption = "(ref:turnover-vs-geodist-model-table)") %>%
  hold_pos_if_latex()
```
