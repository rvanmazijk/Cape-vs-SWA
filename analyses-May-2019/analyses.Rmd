...
Here, I $log(x+1)$-transform and perform a scaled and centred PCA of the roughness values as defined above, at each of five spatial scales: the base data resolution (0.05º x 0.05º), eighth- (EDS), quarter- (QDS), half- (HDS) and three-quarter-degree-squares (3QDS).
...
CLES_results %>%
  filter(variable != "PC2") %>%
  ggplot(aes(resolution, CLES_value)) +
    geom_point() +
...
ggplot(CLES_results2, aes(resolution, CLES_value, colour = variable_type)) +
    geom_point() +
    geom_smooth(
      mapping = aes(group = variable),
      method = lm,
      data = mutate(CLES_results2,
...
knitr::kable(CLES_model_summaries, align = "lrrrcc", caption = paste(
  ...
  "of $CLES$ against each form of environmental roughness (sig = P <= 0.07)."
))
```
...
```{r plot-richness-turnover-univariate}
data_for_histograms <- as_tibble(rbind(
  HDS %>%
    dplyr::select(region, HDS_richness, mul_turnover) %>%
    gather(metric, value, HDS_richness, mul_turnover),
  QDS %>%
    dplyr::select(region, QDS_richness) %>%
    gather(metric, value, QDS_richness)
))
ggplot(data_for_histograms, aes(value, fill = region)) +
...
data_for_histograms %>%
  filter(metric %in% c("HDS_richness", "mul_turnover")) %>%
...
```{r plot-richness-turnover-bivariate}
HDS %>%
