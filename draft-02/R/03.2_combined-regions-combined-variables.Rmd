...
models <- list(m_HDS_richness, m_QDS_richness)
names(models) <- c("HDS_richness", "QDS_richness")
```
...
```{r plot-combined-region-multivariate-models, fig.width=7, fig.height=6, fig.cap="Slope estimates of the three multiple linear regressions of (a) $S_{\\mathrm{HDS}}$ and (b) $S_{\\mathrm{QDS}}$ againt environmental heterogeneity variables interacting with region. Each model was simplified using reverse stepwise regression model selection using $AIC$-scores. Points with error bars denote slope estimates and their 95% confidence intervals. All estimates were significant ($P < 0.05$) unless otherwise stated ($0.10 > P > 0.05$, almost significant, \"AS\"; $P > 0.05$, not significant, \"NS\")."}
