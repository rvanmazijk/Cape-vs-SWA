...
fit_combined_region_multivariate <- function(response,
  ...
  # TODO: Use ols_step_all_possible() etc. in final version
...
m_HDS_richness2      <- fit_combined_region_multivariate("HDS_richness", TRUE)
m_QDS_richness      <- fit_combined_region_multivariate("QDS_richness")
m_QDS_richness2      <- fit_combined_region_multivariate("QDS_richness", TRUE)
m_add_turnover_prop <- fit_combined_region_multivariate("add_turnover_prop")

m_HDS_richness$data <- HDS
m_HDS_richness2$data <- rm_high_richness(HDS)
m_QDS_richness$data <- QDS
m_QDS_richness2$data <- rm_high_richness(QDS)
m_add_turnover_prop$data <- HDS

summary(m_HDS_richness)
summary(m_HDS_richness2)
summary(m_QDS_richness)
summary(m_QDS_richness2)
summary(m_add_turnover_prop)

visreg::visreg(m_HDS_richness,
  xvar    = "Elevation_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
)
