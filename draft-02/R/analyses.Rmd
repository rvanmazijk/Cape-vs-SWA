...
```{r fit-sep-region-step-models}
predictor_names <- HDS %>%
  {names(.)[str_which(names(.), "roughness")]} %>%
  paste(collapse = " + ")
models <- list(
  GCFR_HDS_richness = lm(
    formula = glue("HDS_richness      ~ {predictor_names}"), data = GCFR_HDS
  ),
  GCFR_QDS_richness = lm(
    formula = glue("QDS_richness      ~ {predictor_names}"), data = GCFR_QDS
  ),
  GCFR_QDS_turnover = lm(
    formula = glue("add_turnover_prop ~ {predictor_names}"), data = GCFR_HDS
  ),
  SWAFR_HDS_richness = lm(
    formula = glue("HDS_richness      ~ {predictor_names}"), data = SWAFR_HDS
  ),
  SWAFR_QDS_richness = lm(
    formula = glue("QDS_richness      ~ {predictor_names}"), data = SWAFR_QDS
  ),
  SWAFR_QDS_turnover = lm(
    formula = glue("add_turnover_prop ~ {predictor_names}"), data = SWAFR_HDS
  )
)
# (Forward & reverse ("bi-directional") stepwise regression)
#models %<>% map(step, trace = 0)
```

```{r try-olsrr}
foo <- ols_step_all_possible(models$GCFR_HDS_richness)
best_preds <- foo %>%
  as_tibble() %>%
  ...
  pull(predictors) %>%
  str_replace_all(" ", " + ")
new_foo <- lm(glue("HDS_richness ~ {best_preds}"), data = GCFR_HDS)
tidy(new_foo)
```
...
my_AIC_table <- function(..., caption = "...") {
  AIC(...) %>%
    ...
    mutate(model = c("PC1", "PC1+Region", "PC1:Region")) %>%
    mutate_if(is.numeric, ~format(round(.x, digits = 3), nsmall = 3)) %>%
...
```{r}
# Choose m3
# (ggplot() w/ 2x geom_smooth()s produces same fits as visreg::visreg())
...
```{r}
models <- list(
  lm(HDS_richness      ~ PC1,          HDS),
  lm(HDS_richness      ~ PC1 + region, HDS),
  lm(HDS_richness      ~ PC1 * region, HDS),
  lm(QDS_richness      ~ PC1,          QDS),
  lm(QDS_richness      ~ PC1 + region, QDS),
  lm(QDS_richness      ~ PC1 * region, QDS),
  lm(add_turnover      ~ PC1,          HDS),
  lm(add_turnover      ~ PC1 + region, HDS),
  lm(add_turnover      ~ PC1 * region, HDS),
  lm(add_turnover_prop ~ PC1 ,         HDS),
  lm(add_turnover_prop ~ PC1 + region, HDS),
  lm(add_turnover_prop ~ PC1 * region, HDS)
)
str(models[[1]])
```
