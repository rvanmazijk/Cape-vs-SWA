...
```{r import-cellular-data-again, include=FALSE}
...
knitr::kable(preds, caption = "[Using variables selected in section 3.2.]")
```
...
m_all <- lm(
  glue(
    "HDS_richness ~
      {preds$terms[preds$response == 'HDS_richness']}"
  ),
  HDS
)
summary(m_all)
...
m_all_step %>%
  ...
  mutate(
    n_predictors = map_dbl(str_split(predictors, " "), length),
    delta_aic    = aic - min(aic),
    best_model   = n_predictors == min(n_predictors[delta_aic < 2])
  ) %>%
  arrange(desc(best_model), delta_aic, n_predictors)

#m_all_int <- lm(
#  glue(
#    "HDS_richness ~
#      {preds$terms[preds$response == 'HDS_richness']} +
#      {paste('(', preds$terms[preds$response == 'HDS_richness'], ') * region')}"
#  ),
#  HDS
#)
m_all_int <- lm(
...
summary(m_all_int)
rownames(summary(m_all_int)$coefficients)
#m_all_int_step <- ols_step_all_possible(m_all_int)
m_all_int_step <- step(m_all_int, direction = "both")
summary(m_all_int_step)
rownames(summary(m_all_int_step)$coefficients)
