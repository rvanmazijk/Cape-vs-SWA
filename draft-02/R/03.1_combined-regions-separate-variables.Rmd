...
Tables 4 and 5 present the results of simple linear regressions of each form of environmental heterogeneity separately as predictors of each of the three response variables ($S_{\mathrm{HDS}}$, $S_{\mathrm{QDS}}$ and $T_{\mathrm{QDS}} / S_{\mathrm{HDS}}$ respectively). In each table, the "best" model (sensu $AIC$) was select as the simplest model with $\Delta AIC < 2$---i.e. a more complex model was only justified when it had the lowest $AIC$-score by more than 2 $AIC$-units.
...
compare_combined_region_models_SUMMARY <- function(models, response = NULL) {
  if (is.null(response)) {
...
compare_combined_region_models_APPENDIX <- function(models, response = NULL) {
  if (is.null(response)) {
    response <- unique(models$response)
  }
  ...
  knitr::kable(models_summary_printable,
    caption = glue(
      ...
      while periods (\".\") denote $P$-values > 0.05 but < 0.1."
    ),
    col.names = c(
...
compare_combined_region_models_SUMMARY(combined_region_HDS_models)
```
...
compare_combined_region_models_SUMMARY(combined_region_QDS_models)
```

```{r plot-combined-region-PC1-models, fig.width=6, fig.height=3, fig.cap="Fits of simple linear regressions of (a) $S_{\\mathrm{HDS}}$ and (b) $S_{\\mathrm{QDS}}$ against each respective scale's PC1 scores for the combined-region dataset. These models are described in the last rows of Tables 4 and 5 respectively. Grey bands denote 95% confidence intervals."}
...
```{r explore-combined-region-PC1-model-residuals, eval=FALSE}
...
ggplot(HDS, aes(PC1_model_fit, PC1_model_residual)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey75") +
  geom_point()

ggplot(HDS, aes(lon, lat, fill = PC1_model_residual)) +
  geom_tile() +
  facet_grid(~region, scales = "free") +
  scale_fill_viridis_c()

ggplot(QDS, aes(lon, lat, fill = PC1_model_residual)) +
  geom_tile() +
  facet_grid(~region, scales = "free") +
  scale_fill_viridis_c()
