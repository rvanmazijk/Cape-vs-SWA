...
fit_combined_region_models <- function(response, rm_high_richness_pts=FALSE) {
  stopifnot(exprs = {
...
m12 <- combined_region_HDS_models2 %>%
  filter(region_term == "int", predictor == "MAP_roughness") %>%
  pull(model)
m12[[1]]$data <- rm_high_richness(HDS)
m12_plot <- visreg::visreg(m12[[1]],
  xvar    = "MAP_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m22 <- combined_region_HDS_models2 %>%
  filter(region_term == "add", predictor == "NDVI_roughness") %>%
  pull(model)
m22[[1]]$data <- rm_high_richness(HDS)
m22_plot <- visreg::visreg(m22[[1]],
  xvar    = "NDVI_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m32 <- combined_region_HDS_models2 %>%
  filter(region_term == "add", predictor == "Clay_roughness") %>%
  pull(model)
m32[[1]]$data <- rm_high_richness(HDS)
m32_plot <- visreg::visreg(m32[[1]],
  xvar    = "Clay_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m42 <- combined_region_QDS_models2 %>%
  filter(region_term == "int", predictor == "PDQ_roughness") %>%
  pull(model)
m42[[1]]$data <- rm_high_richness(QDS)
m42_plot <- visreg::visreg(m42[[1]],
  xvar    = "PDQ_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m52 <- combined_region_QDS_models2 %>%
  filter(region_term == "int", predictor == "NDVI_roughness") %>%
  pull(model)
m52[[1]]$data <- rm_high_richness(QDS)
m52_plot <- visreg::visreg(m52[[1]],
  xvar    = "NDVI_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m62 <- combined_region_QDS_models2 %>%
  filter(region_term == "int", predictor == "Soil.C_roughness") %>%
  pull(model)
m62[[1]]$data <- rm_high_richness(QDS)
m62_plot <- visreg::visreg(m62[[1]],
  xvar    = "Soil.C_roughness",
  by      = "region",
  overlay = TRUE,
  gg      = TRUE
...
m2_legend <- get_legend(m2_plots[[1]])
m2_plots %<>% map(~ .x + theme(legend.position = "none"))
m2_plots[1:3] %<>% map(~ .x + ylim(-100, 3100))
m2_plots[4:6] %<>% map(~ .x + ylim(-100, 2600))
...
```{r plot-combined-region-PC1-model-residuals, eval=FALSE}
richness_m <- lm(HDS_richness ~ PC1, HDS)
HDS$predicted_HDS_richness <- richness_m$fitted.values
HDS$residual_HDS_richness <- richness_m$residuals

ggplot(HDS, aes(HDS_richness, predicted_HDS_richness, fill = region)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", alpha = 0.5) +
  geom_point(shape = 21) +
  lims(x = range(HDS$HDS_richness), y = range(HDS$HDS_richness)) +
  scale_fill_manual("Region", values = c("black", "white"))

ggplot(HDS, aes(HDS_richness, residual_HDS_richness, fill = region)) +
  geom_point(shape = 21) +
  scale_fill_manual("Region", values = c("black", "white"))

ggplot(HDS, aes(PC1, residual_HDS_richness, fill = region)) +
  geom_point(shape = 21) +
  scale_fill_manual("Region", values = c("black", "white"))
