...
```{r maps, eval=FALSE}
data %>%
  map(~ ggplot(.x, aes(lon, lat, size = QDS_richness)) +
    geom_point() +
    facet_grid(~region, scales = "free_x")
  ) %>%
  {plot_grid(plotlist = .)}
ggplot(data %$% QDS[QDS$region == "GCFR", ]) +
  aes(lon, lat, size = QDS_richness) +
  geom_path(
    data = GCFR_border,
    aes(long, lat, group = group),
  ...
  geom_point() +
  scale_size_continuous(breaks = c(100, 500, 1000, 2000), range = c(0.1, 4))
ggplot(data %$% QDS[QDS$region == "SWAFR", ]) +
  aes(lon, lat, size = QDS_richness) +
  geom_path(
    data = SWAFR_border,
    aes(long, lat, group = group),
  ...
  geom_point() +
  scale_size_continuous(breaks = c(100, 500, 1000, 1500, 2000), range = c(0.1, 4))

ggplot(data %$% HDS[HDS$region == "GCFR", ]) +
  aes(lon, lat, size = HDS_richness) +
  geom_path(
    data = GCFR_border,
    aes(long, lat, group = group),
    color = "grey50", size = 0.25
  ...
  geom_point() +
  scale_size_continuous(breaks = c(100, 500, 1000, 1500, 2000, 2500), range = c(0.1, 4))
ggplot(data %$% HDS[HDS$region == "SWAFR", ]) +
  aes(lon, lat, size = HDS_richness) +
  geom_path(
    data = SWAFR_border,
    aes(long, lat, group = group),
